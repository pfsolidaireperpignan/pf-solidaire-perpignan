/* ==========================================================================
   GESTION ADMINISTRATIVE & HUB (SCRIPT.JS)
   ========================================================================== */
...
window.logout = function() {
    signOut(auth).then(() => {
        window.location.href = "index.html"; // Recharge propre
...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        if(openId) {
            // Si on vient de la facture pour voir le dossier
            document.getElementById('hub-accueil').classList.add('hidden');
            document.getElementById('app-content').classList.remove('hidden');
            window.openTab('tab-dossier');
            setTimeout(() => window.chargerDossier(openId), 500); // Chargement auto
        } else {
            // Sinon affichage normal du Hub
...
window.retourHub = function() {
    ...
    document.getElementById('hub-accueil').classList.remove('hidden');
    // Nettoyer l'URL si on avait un ?open_id
...
// Rechercher dans la liste (Tableau)
window.rechercherDossier = async function() {
    ...
    try {
            ...
            if (fullName.includes(searchVal)) {
                ...
                tr.innerHTML = `
                    ...
                    <td style="display:flex; gap:5px;">
                        <button class="btn-outline" style="padding:5px 10px;" onclick="window.chargerDossier('${doc.id}')" title="Ouvrir Dossier">
                            <i class="fas fa-folder-open"></i>
                        </button>
                        
                        <button class="btn-purple" style="padding:5px 10px; border:none; border-radius:4px; color:white; cursor:pointer;" 
                                onclick="window.location.href='facturation.html?id=${doc.id}'" title="Aller aux Devis">
                            <i class="fas fa-euro-sign"></i> Facturer
                        </button>

                        <button class="btn-red" style="padding:5px 10px; border:1px solid #fca5a5; background:#fee2e2; color:#dc2626; border-radius:4px; cursor:pointer;" 
                                onclick="window.supprimerClient('${doc.id}')" title="Supprimer">
        ...
        console.error(e);
        tbody.innerHTML = `<tr><td colspan="5">Erreur de connexion</td></tr>`; 
    }
};

// Supprimer un Client
window.supprimerClient = async function(id) {
    if(confirm("ATTENTION : Voulez-vous vraiment supprimer ce dossier et toutes ses informations ?")) {
        try {
...
// Charger un dossier dans le formulaire
window.chargerDossier = async function(id) {
    try {
        ...
        if (docSnap.exists()) {
            ...
            document.getElementById('current-client-name').textContent = (data.nom || "") + " " + (data.prenom || "");
            window.openTab('tab-dossier'); // Force l'affichage de l'onglet dossier
        } else {
            alert("Dossier introuvable (peut-être supprimé ?)");
        }
    } catch (e) { console.error(e); }
};

// Réinitialiser le formulaire (Nouveau)
window.resetDossier = function() {
    if(confirm("Vider le formulaire pour créer un nouveau client ?")) {
        currentDocId = null;
        ...
        document.getElementById('current-client-name').textContent = "Nouveau Dossier";
        window.openTab('tab-dossier');
    }
};

/* ==========================================================================
   3. FONCTIONS D'INTERFACE (UI Helpers)
   ========================================================================== */
...
window.openTab = function(tabName) {
    ...
    // Masquer sous-sections techniques si on change d'onglet
...
/* ==========================================================================
   4. GENERATION DES DOCUMENTS PDF
   ========================================================================== */
